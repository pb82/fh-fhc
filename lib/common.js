...
function strlen(str) {
  ...
  if (typeof str === 'object') s = util.inspect(str);
  var len = 0;
  if (s) len = s.length;
  return len;
}
...
exports.createTableForProperties = function (type, opts, properties) {
  ...
  _.each(properties, function (prop) {
    ...
    if (prop.value.length > max) prop.value = "TRUNCATED TOO LONG TO DISPLAY. Try --json";
...
function createObjectTable(obj) {
  ...
  for (var k in obj) {
    heads.push(k.replace(/(^|\s)([a-z])/g, function (m, p1, p2) {
...
function createTableForAppProps(app) {
  ...
  if (app.legacy === false) {
    return createNVTable(app);   // TODO - this could be much improved!!
  }
