...
var fhc = require("../fhc");
var requestjs = require("request").defaults({'proxy': fhc.config.get("proxy")});
var url = require("url");
...
function createErrorMessage(statusCode, body){
  ...
  var msg = 'Error - not 2xx status code. (' + statusCode + ')\n';
...
function streamFileUpload(params, cb){
  ...
  requestjs(opts, function(err, response, body){
    if(err){
